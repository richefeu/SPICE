...
void writeParticlesVTP(const std::vector<Particle> &particles, float t, const std::string &file_name) {
  ...
  vtkNew<vtkFloatArray> velArr;
  velArr->SetName("velocity");
  velArr->SetNumberOfComponents(3);
  vtkNew<vtkFloatArray> accArr;
  accArr->SetName("acceleration");
  accArr->SetNumberOfComponents(3);
  vtkNew<vtkFloatArray> forceArr;
  forceArr->SetName("force");
  forceArr->SetNumberOfComponents(3);
  vtkNew<vtkFloatArray> radiusArr;
  radiusArr->SetName("radius");
  radiusArr->SetNumberOfComponents(1);
  vtkNew<vtkFloatArray> massArr;
  massArr->SetName("mass");
  massArr->SetNumberOfComponents(1);
...
void writePVD(const std::vector<std::string> &files, const std::vector<float> &times, const std::string &pvdName) {
  ...
  for (size_t i = 0; i < files.size(); ++i) {
    ...
    ofs << "    <DataSet timestep=\"" << std::fixed << std::setprecision(6) << times[i] << "\" file=\"" << files[i]
        << "\"/>\n";
  }
  ofs << "  </Collection>\n";
  ofs << "</VTKFile>\n";
